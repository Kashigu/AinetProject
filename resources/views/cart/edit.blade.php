...
@section('main')
<form method="POST" action="{{ route('cart.update', ['tshirtImage' => $tshirtImage, 'color' => $color, 'tamanho' => $tamanho]) }}">
            ...
            <div class="col-md-6 mt-4">
                <div class="image-container">
                    ...
                    <div id="colorImageContainer" style="position: relative; width: 380px; height: 380px;">
                        @php
                        $cartID = $tshirtImage->id . '-' . $color . '-' . $tamanho;
                        $cor_code = '';
                        @endphp
                        @foreach ($colors as $cor)
                        @if ($cor->name == $cart[$cartID]['color'])
                        @php
                        $cor_code = $cor->code;
                        @endphp
                        @endif
                        ...
                        @endphp
                        @endif
                        <img id="colorImage" src="{{ asset('storage/tshirt_base/'.$cor_code.'.jpg') }}" width="510" height="510" style="position: absolute; top: 0; left: 0;">

                        @if (Auth::user() && (Auth::user()->id == $customer_id || Auth::user()->user_type == 'A'))
                    ...
                    <div class="row">
                        <div class="col-md-3 mt-2 center">
                            <div class="btn-group" role="group">
                                ...
                                <input id="quantity" name="quantidade" min="1" value="{{ $cart[$cartID]['quantidade'] }}" class="form-control center">
                        ...
                        <div class="col-md-2 fim mt-2">
                            <select class="form-select btn btn-dark" name="tamanho">
                                <option value="XS" {{ old('tamanho', $cart[$cartID]['tamanho']) === 'XS' ? 'selected' : '' }}>XS</option>
                                <option value="S" {{ old('tamanho', $cart[$cartID]['tamanho']) === 'S' ? 'selected' : '' }}>S</option>
                                <option value="M" {{ old('tamanho', $cart[$cartID]['tamanho']) === 'M' ? 'selected' : '' }}>M</option>
                                <option value="L" {{ old('tamanho', $cart[$cartID]['tamanho']) === 'L' ? 'selected' : '' }}>L</option>
                                <option value="XL" {{ old('tamanho', $cart[$cartID]['tamanho']) === 'XL' ? 'selected' : '' }}>XL</option>
                            </select>
                        </div>
                        <div class="col-md-2"></div>
                        <div class="col-md-3 fim mt-2">
                            <button class="btn btn-dark" type="submit" style="font-size: 30px;"><i class="fa-solid fa-cart-shopping"></i> Alter
                            </button>
                        </div>
